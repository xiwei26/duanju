	...
	for _, drama := range a.dramas {
		if dramaProvider(drama) == sourceHongguo {
			if endpoint, valid := buildImageURL(bestDramaCover(drama)); valid {
				targets = append(targets, struct{ name, endpoint string }{"红果封面 CDN", endpoint})
				break
			}
		}
	}
		...
		go func() {
			...
			for index := range jobs {
				if index == len(targets) {
					results[index] = a.checkHongguoMedia(ctx, mediaTask)
					continue
				}
...
func (a *UIApp) checkHongguoMedia(ctx context.Context, task *Task) networkCheckResult {
	result := networkCheckResult{Name: "红果实际媒体"}
	if task == nil {
		result.Detail = "暂无红果分集任务，入口可达不代表媒体可下载"
		return result
	}
