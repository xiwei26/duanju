...
func followingFixture(t *testing.T) *UIApp {
	...
	return &UIApp{cfg: Config{dataDir: t.TempDir()}, dramas: []Drama{
		...
		{ID: "hongguo:7000000000000000002", Title: "无图测试小剧场", TotalEpisode: 5},
		{ID: "hongguo:10001", Title: "未提供集数的本地测试"},
	}}
}
...
func TestFollowingPersistsSeparatelyFromPlaybackAndSameNames(t *testing.T) {
	...
	if len(entries) != 2 {
		t.Fatal("same-name dramas with different IDs were combined", entries)
	}
...
func TestFollowingRejectsCrossSiteUnknownAndMalformedRequests(t *testing.T) {
	app := followingFixture(t)
	app.dramas = append(app.dramas, Drama{ID: "hongguo:7000000000000000004", Source: "huangdou", Title: "冲突来源资料"})
	for _, test := range []struct {
		...
		{"unsupported source", "POST", "application/json", "", `{"dramaId":"huangdou:7000000000000000002","saved":true}`, 400},
		{"unsupported source alias", "POST", "application/json", "", `{"dramaId":"huangguoai:10001","saved":true}`, 400},
		{"conflicting cached source", "POST", "application/json", "", `{"dramaId":"hongguo:7000000000000000004","saved":true}`, 404},
