...
func isPlayableDownloadTask(task *UITask) bool {
	return task != nil && isHongguoTask(task.Source) && !task.RemoveRequested && !isChapterPlaceholderTask(task) &&
...
func hasCollectionPlaybackMedia(task Task) bool {
	return isHongguoTask(task) && (isProviderHTTPMediaURL(task.Chapter.VideoURL) || strings.HasPrefix(task.Chapter.VideoURL, "hongguo-cenc://"))
}
