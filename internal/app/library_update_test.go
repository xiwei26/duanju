...
func TestUnifiedUpdateReturnsImmediatelyAndDeduplicatesWork(t *testing.T) {
	...
	app.dramas = append(app.dramas, Drama{ID: "hongguo:790000", Source: sourceHongguo, Title: "已完成的目录条目", OnlineDate: "2026-03-16", Heat: "20", Views: "0", Cover: coverAddressFixture, SortMetadata: &sortMetadataState{Version: sortMetadataVersion}})
	for index := 0; index < 60; index++ {
		app.dramas = append(app.dramas, Drama{ID: fmt.Sprintf("hongguo:%d", 800000+index), Source: sourceHongguo, Title: "历史文字样本", Heat: "20", Views: "0", Cover: coverAddressFixture})
	}
	t.Cleanup(func() {
		releaseCatalog()
		releaseDetail()
