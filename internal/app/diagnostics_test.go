...
func TestDiagnosticLogConcurrentWritesAndRedaction(t *testing.T) {
	...
	for index := 0; index < 50; index++ {
		...
		go func(index int) {
			defer group.Done()
			d.recordDiagnostic(diagnosticEvent{Event: "fixture.failed", Episode: index + 1, Message: "https://user:fixture-password@media.invalid/video?token=fixture-token&key=fixture-media-key&signature=fixture-signature#fixture-fragment"})
		}(index)
	}
	group.Wait()
