...
func (a *UIApp) runSortMetadataQueue(ctx context.Context, done chan struct{}) {
	...
	for {
		...
		for index, drama := range a.dramas {
			...
			if patch.OnlineDate != "" && patch.OnlineDate != drama.OnlineDate || patch.Heat != "" && patch.Heat != drama.Heat || patch.Views != "" && patch.Views != drama.Views || bestDramaCover(patch) != "" && bestDramaCover(patch) != bestDramaCover(drama) {
