...
export function createLibrary(app) {
  ...
  function refreshDrama(id) {
    if (!['hongguo'].includes(String(id).split(':')[0])) return;
...
function filteredDramas() {
  ...
  return dramas.filter(drama => {
    if (source && sourceKey(drama) !== source || channel && categoryName(drama) !== channel) return false;
    return matchesSearch(searchableText(drama), query, terms) || onlineSearchQuery === keyword && onlineSearchIDs.has(drama.id);
  });
}

function rebuildSources(){ rebuildOptions($('sourceSelect'),app.viewer?.sources || ['hongguo'],'全部站源',sourceLabel,false); }
...
function metadataPriorityIDs(){const pending=new Set(dramas.filter(drama=>!drama.sortMetadata||drama.sortMetadata.version!==1||!coverURL(drama)&&!drama.sortMetadata.coverChecked).map(drama=>drama.id));return visibleIDs.filter(id=>pending.has(id)).slice(0,40);}

function updateRefreshLabel(){ const source=$('sourceSelect').value;const hongguo=app.viewer?.sources?.includes('hongguo')!==false&&(!source||source==='hongguo');updateLibraryButton();$('onlineSearchBtn').hidden=!hongguo;if(!hongguo)searchSuggestions.close();$('searchInput').placeholder=hongguo?'筛选剧库，回车联网搜索红果':'搜索剧名、简介或标签';$('onlineSearchBtn').title='联网搜索红果，也可按回车'; }
...
async function loadDramas(update) {
  ...
  try {
    ...
    sourceWarning = result.error ? '红果更新尚未完成，可在“更多”查看状态' : '';
    if (Array.isArray(result.data)) {
      dramas.length = 0;
      byID.clear();
      for (const drama of result.data) {if(sourceKey(drama)!=='hongguo')continue;coverRepair.reconcile(drama);dramas.push(drama); byID.set(drama.id, drama);}
...
function updateLibraryStatus() {
  ...
  const text = [libraryBusyText, onlineSearchMessage || localSearch, searchFilterMessage, sourceWarning, sortMessage].filter(Boolean).join(' · ');
...
function init() {
  ...
  const allowedSources = app.viewer?.sources || ['hongguo'];
