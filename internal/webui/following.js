...
export function createFollowing(app) {
  ...
  function records() {
    const history = new Map((window.JukuHistory.list?.() || []).filter(entry => sourceKey({id: entry.dramaId, source: entry.source}) === 'hongguo').map(entry => [entry.dramaId, entry]));
  ...
  async function refresh() {
    ...
    refreshPromise = (async () => {
      try {
        ...
        entries.clear();
        for (const entry of result.data || []) if (sourceKey({id: entry.dramaId, source: entry.source}) === 'hongguo') entries.set(entry.dramaId, entry);
        loaded = true;
