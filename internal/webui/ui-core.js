...
function sourceKey(drama){ if(drama.source&&normalizeSource(drama.source)!=='hongguo')return '';return /^hongguo:\d{1,32}$/.test(drama.id)?'hongguo':''; }

function sourceLabel(value){ return normalizeSource(value)==='hongguo'?'红果':''; }

function categoryName(dr){ return firstNonEmpty(dr.categoryName,dr.category_name,dr.typeName,dr.type_name,dr.sortName,dr.sort_name,dr.category,dr.categoryNameSnake)||'未分类'; }
